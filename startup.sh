...
${CRE} run -dit --name vpp1 --privileged --cpuset-cpus=$cpus --ulimit memlock=-1 -v /dev/hugepages:/dev/hugepages  \
...
VPP1NS=`${CRE} inspect -f '{{.State.Pid}}' vpp1`
...
${CRE} exec vpp1 ip link del eth0

echo "=========== Setup HW descriptors ==========="
${CRE} exec vpp1 ethtool -G $interface rx 8160 tx 8160

#echo "=========== Setup RSS ==========="
rmmod irdma;
${CRE} exec vpp1 ethtool -L $interface combined 4
${CRE} exec vpp1 ethtool -X $interface equal 4 start 0

echo "=========== Disable busy polling ==========="
${CRE} exec vpp1 bash -c "echo 0 >> /sys/class/net/$interface/napi_defer_hard_irqs"
${CRE} exec vpp1 bash -c "echo 0 >> /sys/class/net/$interface/gro_flush_timeout"

echo "=========== Setup irq affinity to different cores ==========="
${CRE} exec vpp1 ./set_irq_affinity.sh $irq_cpus $interface

echo "=========== Jump into vpp1 ==========="
${CRE} exec -ti vpp1 ./build-root/install-vpp-native/vpp/bin/vpp -c VPP_STARTUP.conf
